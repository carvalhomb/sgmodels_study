...
We then performed an ANOVA test to analyze any differences between the conditions, including dividing the participants by familiarity with games.

Our variables:

* Between-subjects factor: familiarity with games, with three levels (Medium, Medium-high and High)
...
ftable_gamefam <- structable(gamefam ~ coded_answer, droplevels(qualdata))
...
ftable_gamefam_matched <- structable(gamefam ~ coded_answer, droplevels(qualdata_paired))
...
Reversed frequency in which the comments were made, split by familiarity with serious games.


```{r qualitative_data_paired_sgs, echo=FALSE, results = "asis", cache=TRUE}

ftable_sgsfam_matched <- structable(sgsfam ~ coded_answer, droplevels(qualdata_paired))
ftable_sgsfam_matched <- as.data.frame(addmargins(ftable_sgsfam_matched, margin=2))
ftable_sgsfam_matched <- ftable_sgsfam_matched[with(ftable_sgsfam_matched, order(-Sum)), ]

kable(ftable_sgsfam_matched, caption="Frequency of comments, participants who evaluated both methods, by serious game familiarity")
rm(ftable_sgsfam_matched)
